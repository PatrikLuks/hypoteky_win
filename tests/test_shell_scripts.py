...
def test_shell_script_header_and_content(script_path):
    ...
    assert (
        lines[0].strip() == SHEBANG
    ), f"Skript {script_path} musí začínat shebangem {SHEBANG}."
    assert any(
        "#" in l and "skript" in l.lower() for l in lines[:5]
    ), f"Skript {script_path} musí mít krátký popis v komentáři v prvních 5 řádcích."
