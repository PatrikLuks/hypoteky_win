...
class NotifikaceDeadlineTestCase(TestCase):
    def setUp(self):
        ...
        self.poradce = User.objects.create_user(
            username="poradce", email="poradce@example.com", password="testpass"
        )
        ...
        assert (
            self.poradce.userprofile.role == "poradce"
        ), f"Role poradce: {self.poradce.userprofile.role}"
    ...
    def test_deadline_notifikace_neexistujici_email(self):
        ...
        log = NotifikaceLog.objects.filter(
            prijemce="", klient=self.klient, typ="deadline"
        )
