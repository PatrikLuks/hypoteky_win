...
class IsPoradceOrAdmin(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        user = request.user
        print(
            f"[DEBUG has_permission] user={user}, is_staff={user.is_staff}, is_superuser={user.is_superuser}, role={getattr(getattr(user, 'userprofile', None), 'role', None)}"
        )
    ...
    def has_object_permission(self, request, view, obj):
        user = request.user
        print(
            f"[DEBUG has_object_permission] user={user}, is_staff={user.is_staff}, is_superuser={user.is_superuser}, role={getattr(getattr(user, 'userprofile', None), 'role', None)}"
        )
        if not user.is_authenticated:
            return False
        if user.is_staff or user.is_superuser:
