...
class KlientAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.assertEqual(
            response.status_code, 200, f"JWT token nebyl získán: {response.data}"
        )
        self.token = response.data["access"]
        self.client.credentials(HTTP_AUTHORIZATION="Bearer " + self.token)
        self.klient1 = Klient.objects.create(
            jmeno="Jan Novák",
            datum=date.today(),
            vyber_banky="KB",
        ...
        self.klient2 = Klient.objects.create(
            jmeno="Petr Svoboda",
            datum=date.today(),
            vyber_banky="ČSOB",
    ...
    def test_forbidden_for_klient_role(self):
        ...
        response = client2.post(
            "/api/token/", {"username": "klient", "password": "testpass"}, format="json"
        )
    ...
    def test_patch_partial_update(self):
        ...
        response = client2.post(
            "/api/token/",
            {"username": "klient2", "password": "testpass"},
            format="json",
        )
        token2 = response.data["access"]
        client2.credentials(HTTP_AUTHORIZATION="Bearer " + token2)
