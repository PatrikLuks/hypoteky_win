...
def import_klienti_from_csv(file: IO, encoding: str = 'utf-8') -> int:
    ...
    for idx, row in enumerate(reader):
        ...
        if not created:
            ...
            klient.vyber_banky = row.get('vyber_banky') or klient.vyber_banky
            klient.navrh_financovani_castka = castka if castka is not None else klient.navrh_financovani_castka
            klient.duvod_zamitnuti = row.get('duvod_zamitnuti') or klient.duvod_zamitnuti
            klient.co_financuje = row.get('co_financuje') or klient.co_financuje
            klient.navrh_financovani = row.get('navrh_financovani') or klient.navrh_financovani
