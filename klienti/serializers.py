...
class KlientSerializer(serializers.ModelSerializer):
    ...
    vyber_banky = serializers.CharField(max_length=255, required=False, allow_blank=False)
    navrh_financovani_castka = serializers.DecimalField(max_digits=12, decimal_places=2, required=False, min_value=10000, max_value=100000000)

    def validate_jmeno(self, value):
        if value is not None and not value.strip():
            raise serializers.ValidationError("Jméno klienta nesmí být prázdné.")
        if value is not None and len(value) > 100:
            raise serializers.ValidationError("Jméno klienta je příliš dlouhé (max 100 znaků).")
        return value
    ...
    def validate(self, data):
        ...
        if not self.partial:
            if not data.get('jmeno') or not data['jmeno'].strip():
                raise serializers.ValidationError({"jmeno": "Jméno klienta je povinné."})
            ...
            if not data.get('navrh_financovani_castka'):
                raise serializers.ValidationError({"navrh_financovani_castka": "Zadejte částku návrhu financování."})
        return data
