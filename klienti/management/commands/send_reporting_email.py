...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        User = get_user_model()
        prijemci = list(User.objects.filter(userprofile__role__in=['manažer', 'administrátor'], is_active=True).values_list('email', flat=True))
        if not prijemci:
            self.stdout.write(self.style.WARNING('Žádní příjemci reportu (role manažer/administrátor)'))
            return
        # Data pro reporting
