...
# Najdi všechny uživatele s rolí 'klient' a 'poradce'
klient_users = list(User.objects.filter(userprofile__role='klient'))
poradci_users = list(User.objects.filter(userprofile__role='poradce'))
...
# Každý uživatel s rolí 'klient' dostane jednoho svého klienta
for user in klient_users:
    ...
    print(f"Vytvořen klient: {jmeno} pro uživatele {user.username}")

# Vytvoř vzorové klienty bez přiřazeného uživatele (pro test poradce)
pocet_anonymnich = max(3, len(poradci_users))
for i in range(pocet_anonymnich):
    datum = fake.date_between(start_date='-7M', end_date='today')
    jmeno = fake.name()
    cena = round(random.uniform(1500000, 12000000), 2)
    procento = round(random.uniform(60, 90), 2)
    ...
    for idx, krok in enumerate(kroky):
        ...
        if idx < dalsi_krok_index:
            ...
            klient_kwargs[splneno_field] = None
    klient = Klient.objects.create(**klient_kwargs)
    print(f"Vytvořen klient: {jmeno} (bez uživatele, pouze pro poradce)")

print("Hotovo! Každý uživatel s rolí 'klient' má svého klienta, poradce vidí i anonymní vzory.")
